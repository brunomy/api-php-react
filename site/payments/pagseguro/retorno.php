...
if (!defined('TOKEN'))
    define('TOKEN', 'D321EFB5EA124786B70910F778106050');

function getMensagemByStatus($id) {
    global $product, $sistema;
    $query = $product->DB_fetch_array("SELECT A.*, C.nome nome_pagseguro, C.id id_pagseguro FROM tb_pedidos_status A INNER JOIN tb_pedidos_status_has_tb_pedidos_status_pagseguro B ON B.id_pedido_status = A.id INNER JOIN tb_pedidos_status_pagseguro C ON C.id = B.id_pagseguro_status WHERE C.status = '$id'");
    if ($query)
        return $query->rows[0];
}
...
function retorno_automatico($VendedorEmail, $TransacaoID, $Referencia, $TipoFrete, $ValorFrete, $Anotacao, $DataTransacao, $TipoPagamento, $StatusTransacao, $CliNome, $CliEmail, $CliEndereco, $CliNumero, $CliComplemento, $CliBairro, $CliCidade, $CliEstado, $CliCEP, $CliTelefone, $produtos, $NumItens) {
    ...
    $infostatus = getMensagemByStatus(str_replace("á", "a", utf8_encode($StatusTransacao)));
    ...
    switch (str_replace("á", "a", utf8_encode($StatusTransacao))) {
        ...
        case 'Aprovado':
            ...
            $idvannerusuarioid = explode("|", $Referencia);

            for ($i = 0; $i < count($idvannerusuarioid) - 1; $i++) {
                $form->id_pedido = $idvannerusuarioid[$i];
                $form->status_pagamento = 1;
                $form->metodo_pagamento_id = $TransacaoID;
                $form->tipo_pagamento = $TipoPagamento;
                $form->id_status = $infostatus['id'];

                $result = $product->confirmadoPagseguro($form);
            }
        ...
        case 'Aguardando Pagto': case 'Em Analise':
            ...
            for ($i = 0; $i < count($idvannerusuarioid) - 1; $i++) {
                ...
                $result = $product->atualizarPedidoVoltaPagseguro($form);
            }

            break;
    }

    $idvannerusuarioid2 = explode("|", $Referencia);
    ...
    for ($i = 0; $i < count($produtos); $i++) {
        $valTotal += $product->str2num($produtos[$i]['ProdValor']) * $produtos[$i]['ProdQuantidade'];


        $valor = number_format($product->str2num($produtos[$i]['ProdValor']), 2, ",", ".");

        $msg .= "<strong>Pedido: </strong>" . ($idvannerusuarioid2[0]) . "<br/ >";
        $msg .= "<strong>Produto: </strong>" . ($produtos[$i]['ProdDescricao']) . "<br/ >";
        $msg .= "<strong>Valor: </strong> R$ " . ($valor) . "<br/ >";
        $msg .= "<br />";
        $valor = "";
        $itens += $produtos[$i]['ProdQuantidade'];
    }
    $valTotal = $valTotal;
    $msg .= "<br />";
    $msg .= "<strong>Número de Ítens:</strong> " . $itens . "<br />";
    $msg .= "<strong>Valor Total:</strong> R$ " . number_format($valTotal, 2, ",", ".") . "<br /><br />";

    if ($infostatus['enviar_email']) {

        $to[] = array("email" => $CliEmail, "nome" => utf8_decode($CliNome));
        ...
        $setFrom = '';
        $notificar_vendedor = $sistema->DB_fetch_array('SELECT * FROM tb_pedidos_pedidos WHERE id = '.$idvannerusuarioid2[0].' AND id_vendedor IS NOT NULL AND id_vendedor <> 0');
        ...
        $msg = $sistema->trataTextoNotificar($msg);

        $msg = str_replace("[({ID})]", $idvannerusuarioid2[0], $msg);
        $msg = str_replace("[({NOME})]", $CliNome, $msg);

        $mensagem = str_replace("[[corpo]]", $msg, $mensagem);

        
        $sistema->enviarEmail($to, $setFrom, utf8_decode($assunto), utf8_decode($mensagem));
    }
}
...
header("Location: http://www.realpoker.com.br");
?>
