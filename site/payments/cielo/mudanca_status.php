...
function retorno_automatico($notificacao) {

    global $product, $sistema;
    

    $pedido = $product->pedidoByOrderNumberCielo($notificacao['order_number'], ((float) $notificacao['amount']) / 100);
    ...
    $statusZerado = $product->zerarStatusTransacaoCielo($notificacao);

    $notificacao['dt_hora'] = date('Y-m-d H:i:s');

    $statusCadastrado = $product->cadastrarStatusTransacaoCielo($notificacao);

    $formPedido = new stdClass();
    $formPedido->id = $pedido->id;
    $formPedido->metodo_pagamento_id = $notificacao['checkout_cielo_order_number'];



    switch ($notificacao["payment_method_type"]) {
    ...
    switch ($notificacao["payment_method_brand"]) {
    ...
    if ($statusCadastrado->result) {

        $form = new \stdClass;

        $infostatus = getMensagemByStatus($notificacao['payment_status']);
        ...
        $sistema->inserirRelatorio('Debug cielo enviar email 1');
...
if ($_POST) {
    ...
    $sistema->inserirRelatorio("mudança status: " . json_encode($_POST));
