<?php
declare(strict_types=1);

use Phinx\Migration\AbstractMigration;

final class CreateProdutosCategorias extends AbstractMigration
{
    /**
     * Change Method.
     *
     * Write your reversible migrations using this method.
     *
     * More information on writing migrations is available here:
     * https://book.cakephp.org/phinx/0/en/migrations.html#the-change-method
     *
     * Remember to call "create()" or "update()" and NOT "save()" when working
     * with the Table class.
     */
    public function change(): void
    {

        $table = $this->table('tb_produtos_categorias', ['collation' => 'utf8mb4_swedish_ci']);
        $table  ->addColumn('id_pai',               'integer',  ['null'=>true])
                ->addColumn('nome',                 'string',   ['null'=>true,'limit'=>100])
                ->addColumn('descricao',            'string',   ['null'=>true])
                ->addColumn('resumo',               'text',     ['null'=>true])
                ->addColumn('ordem',                'integer',  ['null'=>true])
                ->addColumn('stats',                'integer',  ['null'=>true])
                ->addColumn('data',                 'timestamp',['null'=>true,'default'=>'CURRENT_TIMESTAMP'])
                ->addForeignKey('id_pai', 'tb_produtos_categorias', 'id', ['update'=>'SET_NULL', 'delete'=>'NO_ACTION'])
                ->create();

    }
}
