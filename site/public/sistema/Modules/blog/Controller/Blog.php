...
class Blog extends Bootstrap {
    ...
    function __construct() {
        ...
        array_push($this->crop_imagem_sizes, array("width" => 500, "height" => 280));
    ...
    private function indexAction() {
        ...
        $this->list = $this->DB_fetch_array("SELECT A.*, B.nome categoria FROM $this->table A INNER JOIN tb_institucional_categorias B ON B.id = A.id_categoria WHERE A.id_categoria = 2 ORDER BY A.nome");
    ...
    private function delAction() {
        ...
        $this->inserirRelatorio("Apagou página blog: [" . $dados->rows[0]['nome'] . "] id: [$id]");
                ...
                if ($seo_request->response) {
                    ...
                    if ($formulario->id == "") {
                        ...
                        if ($query->query) {
                            ...
                            $this->inserirRelatorio("Cadastrou blog: [" . $data->nome . "]");
                        } else {
                            $resposta->type = "error";
                        ...
                        if ($query) {
                            ...
                            $this->inserirRelatorio("Alterou blog: [" . $data->nome . "]");
                        } else {
                            $resposta->type = "error";
                            $resposta->message = "Aconteceu um erro no sistema, favor tente novamente mais tarde!";
                        }
    ...
    private function datatableAction() {
        ...
        $aColumns = array('A.id','A.data','A.nome', 'A.stats');
        ...
        $sTable = "$this->table A INNER JOIN tb_institucional_categorias B ON B.id = A.id_categoria ";

        //declarar condições extras
        $sWhere = "WHERE A.id_categoria = 2 ";
        ...
        if ($rResult) {
            foreach ($rResult as $aRow) {
                ...
                //ID
                $row[] = "<div align=left><a href='blog/edit/id/". $aRow['id'] ."'>" . $aRow ['id'] . "</a></div>";

                //NOME
                $row[] = "<div align=left><a href='blog/edit/id/". $aRow['id'] ."'>" . $aRow ['data'] . "</a></div>";

                //NOME
                $row[] = "<div align=left><a href='blog/edit/id/". $aRow['id'] ."'>" . $aRow ['nome'] . "</a></div>";
                ...
                $row[] = '<div align="left"><a href="blog/edit/id/'. $aRow['id'] .'"><i class="s12 icomoon-icon-pencil"></i></a> '.$excluir.'</div>';
