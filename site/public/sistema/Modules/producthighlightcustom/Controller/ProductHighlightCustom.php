...
ProductHighlightCustom::setAction();

class ProductHighlightCustom extends Bootstrap {

    public $module = "";
    public $permissao_ref = "produtos-produtos";
    public $table = "tb_produtos_destaques_personalizados";
    ...
    function __construct() {
        ...
        $this->module_link = "producthighlightcustom";
        $this->module_title = "Destaque de Produtos Personalizados";
        $this->retorno = "producthighlightcustom";
    }
    ...
    private function indexAction() {
        ...
        $this->list = $this->DB_fetch_array("SELECT A.*, IFNULL(C.nome, B.nome) nome FROM tb_produtos_destaques_personalizados A INNER JOIN tb_produtos_produtos B ON B.id = A.id_produto LEFT JOIN tb_produtos_personalizados C ON C.id = A.id_personalizado ORDER BY A.ordem");
    ...
    private function editAction() {
        ...
        if ($this->id == "") {
            ...
            $this->produtos = $this->DB_fetch_array("SELECT A.id id_personalizado, A.id_produto, A.nome, C.id FROM tb_produtos_personalizados A INNER JOIN tb_produtos_produtos B ON B.id = A.id_produto AND A.apagado != 1 AND B.apagado != 1 LEFT JOIN tb_produtos_destaques_personalizados C ON C.id_personalizado = A.id AND C.id = 0");
            ...
            $this->produtos = $this->DB_fetch_array("SELECT A.id id_personalizado, A.id_produto, A.nome, C.id FROM tb_produtos_personalizados A INNER JOIN tb_produtos_produtos B ON B.id = A.id_produto AND A.apagado != 1 AND B.apagado != 1 LEFT JOIN tb_produtos_destaques_personalizados C ON C.id_personalizado = A.id AND C.id = {$this->registro['id']}");
        ...
        if ($rows->num_rows) {
            foreach ($rows->rows as $row) {
                if (!in_array($row['id_produto'], $ids)) {
                    $query = $this->DB_fetch_array("SELECT A.id id_produto, A.nome, B.id FROM tb_produtos_produtos A LEFT JOIN tb_produtos_destaques_personalizados B ON B.id_produto = A.id AND B.id_personalizado IS NULL AND B.id = $idDestaque WHERE A.id = {$row['id_produto']}");
    ...
    private function validaFormulario($form) {
        ...
        if (isset($form->id_personalizado) && $form->id_personalizado != "") {
            ...
            if ($form->id == "") {
                $verifica = $this->DB_fetch_array("SELECT * FROM tb_produtos_destaques_personalizados WHERE id_personalizado $id_personalizado AND id_produto = $id_produto ");
            } else {
                $verifica = $this->DB_fetch_array("SELECT * FROM tb_produtos_destaques_personalizados WHERE id_personalizado $id_personalizado AND id_produto = $id_produto AND id != $form->id ");
            }
        }
