...
class EletronicPoint extends Bootstrap {
    ...
    private function editAction() {
        ...
        if ($this->id == "" AND $_SERVER['REQUEST_METHOD'] != 'POST') {
            ...
            $this->referencias = array("entrada" => "Entrada", "saida_almoco" => "Ida ao almoço", "volta_almoco" => "Volta do almoço", "saida" => "Saída");
        ...
        $this->funcionarios = $this->DB_fetch_array("SELECT * FROM tb_admin_users");
    ...
    private function saveAction() {
        ...
        if (!$validacao->return) {
            ...
            if ($formulario->id == "") {
                ...
                if($aux == "saida_almoco")
                    $aux = "almoco";
                else if($aux == "volta_almoco")
                    $aux = "retorno";
                $aux = "hora_".$aux;

                $data->ip = "Lançado manualmente";
                $query = $this->DB_fetch_array("SELECT almoco, $aux FROM tb_admin_users WHERE id = ".$data->id_user)->rows[0];
                $data->almoco = $query['almoco'];
                $data->hora_contrato = $query[$aux];
                ...
                $data->minutos_dia = $this->horaParaMinutos($hour->rows[0]['hora']);;
    ...
    private function exportAction() {
        ...
        header("Content-Disposition: attachment; filename=ponto-eletronico-" . date("Y-m-d") . ".html");
    ...
    public function minutosParaHora($minutos){
        ...
        if($minutos%60 != "" AND $horas < 0)
            $horas++;
        if($horas != 0)
            return str_pad($horas, 2, "0", STR_PAD_LEFT).":".abs($minutos%60).":00";
        else
