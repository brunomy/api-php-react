...
class User extends Bootstrap {
    ...
    public $module = "";
    public $permissao_ref = "admin-users";
    ...
    private function editAction() {
        ...
        if ($this->id == "") {
            ...
            $this->funcoes = $this->DB_fetch_array("SELECT B.id_usuario, A.id, A.nome, B.ler, B.gravar, B.excluir, B.editar FROM tb_admin_funcoes A LEFT JOIN tb_user_permissoes B ON A.id=B.id_funcao ORDER BY A.nome");
            ...
            $this->funcoes = $this->DB_fetch_array("SELECT B.id_usuario, A.id, A.nome, B.ler, B.gravar, B.excluir, B.editar FROM tb_admin_funcoes A LEFT JOIN tb_user_permissoes B ON A.id=B.id_funcao AND B.id_usuario = $this->id ORDER BY A.nome");
            $this->registro['pi'] = "";
        ...
        $this->contaazul_sellers = $contaazul->listSellers();
    ...
    private function setPermissions($idUsuario, $post) {
        ...
        foreach ($funcoes->rows as $funcao) {
            ...
            //$this->DB_insert("tb_user_permissoes", "id_usuario,id_funcao,ler,gravar,editar,excluir", $formulario->id.",".$funcao['id'].",$ler,$gravar,$editar,$excluir");
        }

        $this->DB_connect();
        $this->mysqli->query("INSERT INTO tb_user_permissoes (id_usuario,id_funcao,ler,gravar,editar,excluir) VALUES " . implode(',', $insertData));
        $this->DB_disconnect();
    }
    ...
    private function validaFormulario($form) {
        ...
        if ($form->nome == "") {
            ...
            $resposta->return = false;
            return $resposta;
        } else if($form->bate_ponto == 1){
            if (isset($form->almoco) && $form->almoco == 1 && ($form->hora_almoco == "" || $form->hora_retorno == "")) {
                $resposta->type = "validation";
                $resposta->message = "Preencha este campo";
                $resposta->field = ($form->hora_almoco == "") ? "hora_almoco" : "hora_retorno";
                $resposta->return = false;
                return $resposta;
            } else if ($form->hora_entrada == "") {
                $resposta->type = "validation";
                $resposta->message = "Preencha este campo";
                $resposta->field = "hora_entrada";
                $resposta->return = false;
                return $resposta;
            } else if (!$this->checktime($form->hora_entrada)) {
                $resposta->type = "validation";
                $resposta->message = "Preencha este campo com um horário válido";
                $resposta->field = "hora_entrada";
                $resposta->return = false;
                return $resposta;
            } else if (isset($form->almoco) && $form->almoco == 1 && !$this->checktime($form->hora_almoco)) {
                $resposta->type = "validation";
                $resposta->message = "Preencha este campo com um horário válido";
                $resposta->field = "hora_almoco";
                $resposta->return = false;
                return $resposta;
            } else if (isset($form->almoco) && $form->almoco == 1 && !$this->checktime($form->hora_retorno)) {
                $resposta->type = "validation";
                $resposta->message = "Preencha este campo com um horário válido";
                $resposta->field = "hora_retorno";
                $resposta->return = false;
                return $resposta;
            } else if ($form->hora_saida == "") {
                $resposta->type = "validation";
                $resposta->message = "Preencha este campo";
                $resposta->field = "hora_saida";
                $resposta->return = false;
                return $resposta;
            } else if (!$this->checktime($form->hora_saida)) {
                $resposta->type = "validation";
                $resposta->message = "Preencha este campo com um horário válido";
                $resposta->field = "hora_saida";
                $resposta->return = false;
