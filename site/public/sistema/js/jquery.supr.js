...
(function ($) {
    ...
    $.supr = function (element, options) {
        ...
        plugin.setCurrentNav = function () {
            ...
            if (domain === '') {
                ...
                if (plugin.settings.sideNav.absoluteUrl) {
                    ...
                    this.setCurrentClass(navLinks, afterDomain, module);
        ...
        plugin.setCurrentClass = function (mainNavLinkAll, url, module) {
            var flag = true;
            ...
            mainNavLinkAll.each(function (index) {
                ...
                //var hasModule = href.toLowerCase();
                ...
                if (href === url) {
                    ...
                    var href = $(".sidenav").find('a[href="' + url + '"]');
                    href.parents(".sub").addClass('show');
                    //href.addClass('active');
                    href.append("<span class='indicator'></span>");

                    flag = false;
                }
                //implementado por Rafael
                else if (href == module) {
                    $(this).parents().closest('.sub').addClass('show');
                    $(this).addClass('active');
                    if (plugin.settings.header.fixed && store.get('fixed-header') == 1) {
                        $(this).append("<span class='indicator'></span>");
                    }
                    ...
                    var interna = module + "/edit/id/" + url;
                    if (href == interna) {
                        $(this).parents().closest('.sub').addClass('show');
                        $(this).addClass('active');
                        if (plugin.settings.header.fixed && store.get('fixed-header') == 1) {
                        ...
                        var new_interna = window.location.pathname.split('/');
                        new_interna = new_interna[new_interna.length - 2];
                        ...
                        if (href == new_interna) {
                            ...
                            new_interna = window.location.pathname.split('/');
                            new_interna = new_interna[new_interna.length - 6];
            ...
            if (flag) {
                var href = $(".sidenav").find('a[href^="' + url + '"]');
                href.parents().closest('.sub').addClass('show');
                //href.addClass('active');
                href.append("<span class='indicator'></span>");
            }
        }
        ...
        plugin.panels = function () {
            ...
            if (!$('.contentwrapper').hasClass('notSortable')) {
                ...
                $('.reset-layout').click(function (e) {
                    bootbox.confirm({
                        message: "Aviso!!! Esta ação irá repor a posição do painel",
        ...
        plugin.waitMe = function () {
            (function (e) {
                e.fn.waitMe = function (t) {
                    return this.each(function () {
                        function m() {
                            n.removeClass(r + "_container");
                            n.find("." + r).remove()
                        }
                        var n = e(this), r = "waitMe", i, s, o, u = false, a = "background-color", f = "", l = "", c, h, p, d;
                        var v = {init: function () {
                                function m() {
                                    c = e('<div class="' + r + '"></div>');
                                    var t = "width:" + d.sizeW + ";height:" + d.sizeH;
                                    switch (d.effect) {
                                        case"none":
                                            o = 0;
                                            break;
                                    ...
                                    if (o > 0) {
                                        s = e('<div class="' + r + "_progress " + d.effect + '"></div>');
                                        for (var v = 1; v <= o; ++v) {
                                            if (u) {
                                                l += '<div class="' + r + "_progress_elem" + v + '" style="' + p + '"><div style="' + a + ":" + d.color + '"></div></div>'
                                            } else {
                                                l += '<div class="' + r + "_progress_elem" + v + '" style="' + a + ":" + d.color + ";" + p + '"></div>'
                                            }
                                        }
                                        s = e('<div class="' + r + "_progress " + d.effect + '" style="' + h + f + '">' + l + "</div>")
                                    }
                                    if (d.text) {
                                        i = e('<div class="' + r + '_text" style="color:' + d.color + '">' + d.text + "</div>")
                                    }
                                    if (n.find("> ." + r)) {
                                        n.find("> ." + r).remove()
                                    }
                                    waitMeDivObj = e('<div class="' + r + '_content"></div>');
                                    waitMeDivObj.append(s, i);
                                    c.append(waitMeDivObj);
                                    if (n[0].tagName == "HTML") {
                                        n = e("body")
                                    }
                                    n.addClass(r + "_container").append(c);
                                    n.find("> ." + r).css({background: d.bg});
                                    n.find("." + r + "_content").css({marginTop: -n.find("." + r + "_content").outerHeight() / 2 + "px"})
                                }
                                var v = {effect: "bounce", text: "", bg: "rgba(255,255,255,0.7)", color: "#000", sizeW: "", sizeH: ""};
                                d = e.extend(v, t);
                                m()
                            }, hide: function () {
                                m()
                        ...
                        if (v[t]) {
                            return v[t].apply(this, Array.prototype.slice.call(arguments, 1))
                        } else if (typeof t === "object" || !t) {
                            return v.init.apply(this, arguments)
                        }
                        e.event.special.destroyed = {remove: function (e) {
                                if (e.handler) {
                                    e.handler()
                                }
                            }}
        ...
        plugin.breadCrumbs = function () {
            /*
             ...
             //empty curmb
             breadcrumb.empty();
             breadcrumb.append('<li>Você está aqui:</li>');
             breadcrumb.append('<li><a href="dashboard" class="tip" title="voltar para o painel">' + homeIcon + '</a></li>');
             ...
             text = navel.children('.notification').remove().end().text();
             if (text == "") {
             var migalhas_modulo = window.location.pathname;
             
             migalhas_modulo = migalhas_modulo.split("/");
             
             var module = migalhas_modulo[migalhas_modulo.length - 1];
             
             text = $('a[href$="' + module + '"]:first').text();
             
             if (text == "") {
             module = migalhas_modulo[migalhas_modulo.length - 2];
             text = $('a[href$="' + module + '"]:first').text();
             }
        ...
        plugin.checkBoxesAndRadios = function () {
            ...
            var radios = $('input[type=radio]:not(.no_script)');
        ...
        plugin.waitMe = function () {
            /* waitMe - 12.05.14 */
            (function (e) {
                e.fn.waitMe = function (g) {
                    return this.each(function () {
                        var f = e(this), r, l, b, m = !1, n = "background-color", h = "", p, d, c, a, q = {init: function () {
                                a = e.extend({effect: "bounce", text: "", bg: "rgba(255,255,255,0.7)", color: "#000", sizeW: "", sizeH: ""}, g);
                                p = e('<div class="waitMe"></div>');
                                switch (a.effect) {
                                    case "none":
                                        b = 0;
                                        break;
                                    case "bounce":
                                        b = 3;
                                        d = "";
                                        c = "width:" + a.sizeW + ";height:" + a.sizeH;
                                        break;
                                    case "rotateplane":
                                        b = 1;
                                        d = "";
                                    ...
                                    case "stretch":
                                        b = 5;
                                        d = "";
                                        c = "width:" + a.sizeW + ";height:" +
                                                a.sizeH;
                                        break;
                                    case "orbit":
                                        b = 2;
                                        d = "width:" + a.sizeW + ";height:" + a.sizeH;
                                        c = "";
                                        break;
                                    case "roundBounce":
                                        b = 12;
                                    ...
                                    case "rotation":
                                        b = 1, n = "border-color",
                                                d = "", c = "width:" + a.sizeW + ";height:" + a.sizeH
                                }
                                "" == a.sizeW && "" == a.sizeH && (d = c = "");
                                if (0 < b) {
                                    l = e('<div class="waitMe_progress ' + a.effect + '"></div>');
                                    for (var k = 1; k <= b; ++k)
                                        h = m ? h + ('<div class="waitMe_progress_elem' + k + '" style="' + c + '"><div style="' + n + ":" + a.color + '"></div></div>') : h + ('<div class="waitMe_progress_elem' + k + '" style="' + n + ":" + a.color + ";" + c + '"></div>');
                                    l = e('<div class="waitMe_progress ' + a.effect + '" style="' + d + '">' + h + "</div>")
                                }
                                a.text && (r = e('<div class="waitMe_text" style="color:' + a.color + '">' + a.text +
                                        "</div>"));
                                f.find("> .waitMe") && f.find("> .waitMe").remove();
                                waitMeDivObj = e('<div class="waitMe_content"></div>');
                                waitMeDivObj.append(l, r);
                                p.append(waitMeDivObj);
                                "HTML" == f[0].tagName && (f = e("body"));
                                f.addClass("waitMe_container").append(p);
                                f.find("> .waitMe").css({background: a.bg});
                                f.find(".waitMe_content").css({marginTop: -f.find(".waitMe_content").outerHeight() / 2 + "px"})
                            }, hide: function () {
                                f.removeClass("waitMe_container");
                                f.find(".waitMe").remove()
                            }};
                        if (q[g])
                            return q[g].apply(this, Array.prototype.slice.call(arguments,
                                    1));
                        if ("object" === typeof g || !g)
                            return q.init.apply(this, arguments);
                        e.event.special.destroyed = {remove: function (a) {
                                a.handler && a.handler()
                            }}
                    })
                }
